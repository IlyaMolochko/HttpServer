...
bool SendStaticDirectory(struct THttpResponse* response, const char* path){
    ...
    TStringBuilder_AppendCStr(&response->Body, INDEX_TEMPLATE_HEADER);
    TStringBuilder_Sprintf(&response->Body, "<h2>%s</h2>\n", path);

    struct dirent *ent;
    while ((ent = readdir(dir)) != NULL) {
        if (strcmp(ent->d_name, ".") != 0 && strcmp(ent->d_name, "..") != 0){
            TStringBuilder_Sprintf(&response->Body, "<a href=\"/%s/%s\">%s</a><br/>", path + 1, ent->d_name, ent->d_name);
        }
    }
    TStringBuilder_AppendCStr(&response->Body, INDEX_TEMPLATE_FOOTER);
    closedir (dir);
